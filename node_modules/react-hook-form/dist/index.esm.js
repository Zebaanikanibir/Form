...
function useForm({ mode = VALIDATION_MODE.onSubmit, reValidateMode = VALIDATION_MODE.onChange, resolver, context, defaultValues = {}, shouldFocusError = true, shouldUnregister, criteriaMode, } = {}) {
    ...
    const executeResolverValidation = React.useCallback(async (names) => {
        const { errors } = await resolver(getFieldsValues(fieldsRef), contextRef.current, getResolverOptions(namesRef.current.mount, fieldsRef.current, criteriaMode));
        if (names) {
    ...
    const updateValidAndValue = (name, ref) => {
        const field = get(fieldsRef.current, name);
    ...
    const updateIsValid = React.useCallback(async (values = {}) => {
        const isValid = resolver
            ? isEmptyObject((await resolver(Object.assign(Object.assign({}, getFieldsValues(fieldsRef)), values), contextRef.current, getResolverOptions(namesRef.current.mount, fieldsRef.current, criteriaMode))).errors)
            : await validateForm(fieldsRef.current, true);
        isValid !== formStateRef.current.isValid &&
            subjectsRef.current.state.next({
                isValid,
    ...
    const handleChange = React.useCallback(async ({ type, target, target: { value, type: inputType } }) => {
        ...
        if (field) {
            ...
            if (resolver) {
                const { errors } = await resolver(getFieldsValues(fieldsRef), contextRef.current, getResolverOptions([name], fieldsRef.current, criteriaMode));
    ...
    const handleSubmit = React.useCallback((onValid, onInvalid) => async (e) => {
        ...
        try {
            if (resolver) {
                const { errors, values } = await resolver(fieldValues, contextRef.current, getResolverOptions(namesRef.current.mount, fieldsRef.current, criteriaMode));
                formStateRef.current.errors = errors;
                fieldValues = values;
            }
            else {
